    ...
    /* USER CODE BEGIN 3 */
	  // 1. Lógica de tiempo: Incrementar el contador cada 100ms
	      if (HAL_GetTick() - ultimo_tiempo >= 100) {
	          ...
	          // Ejemplo: 125 -> centenas: 1, decenas: 2, unidades: 5
	          valores_display[0] = (contador_global / 100) % 10; // Centena
	          valores_display[1] = (contador_global / 10) % 10;  // Decena
	          valores_display[2] = contador_global % 10;         // Unidad
	      }

	      // 3. MUX SCAN: Esto debe ejecutarse SIEMPRE y lo más rápido posible
	      Display_Mux_Scan();
  }
  /* USER CODE END 3 */
}

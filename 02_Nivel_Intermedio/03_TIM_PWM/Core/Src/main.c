...
/* --- Variables de control (globales o estáticas) --- */
uint16_t pwm_duty = 0;       // Ciclo de trabajo actual
int8_t fade_step = 5;        // Velocidad del cambio
...
void Task_BreathingLED(void);
void Task_RGBHandler(void);
...
/* --- Definicion de las Tareas del Planificador --- */
void Task_Heartbeat(void) {
    HAL_GPIO_TogglePin(usr_ledRojo_GPIO_Port, usr_ledRojo_Pin);
}

void Task_BreathingLED(void) {
    // Aquí vive tu lógica del CH4 del TIM3
    Tarea_LED_Breathing();
}

void Task_RGBHandler(void) {
    // Aquí vive tu driver encapsulado
    RGB_Effects_Handler(&ledRGB, 30, 5);
}


/* --- Definicion de Tareas Auxiliares --- */
...
int main(void)
{
	...
	// 3. Iniciamos el efecto arcoíris (brillo al 70%)
	RGB_Start_Effect_Rainbow(&ledRGB, 700);

	//***********************Timer que maneja el led externo***********************
	// Encendemos el timer que maneja el usr_led
...
/*
		...
		// TAREA 1: LED que late (cada 15ms para suavidad)
		static uint32_t lastTick_Breathing = 0;
		if (currentTick - lastTick_Breathing >= 15) {
			lastTick_Breathing = currentTick;
			Tarea_LED_Breathing(); // La función que ya escribimos para el CH4
		}

		// TAREA 2: Motor de efectos RGB (cada 30ms para un arcoíris fluido)
		static uint32_t lastTick_RGB = 0;
		if (currentTick - lastTick_RGB >= 30) {
			lastTick_RGB = currentTick;
			// El driver internamente maneja sus estados
			RGB_Effects_Handler(&ledRGB, 30, 5);
		}

		// TAREA 3: Toggle Led Rojo (cada 30ms para un arcoíris fluido)
		static uint32_t lastTick_Toggle = 0;
		if (currentTick - lastTick_Toggle >= 100) {
			lastTick_Toggle = currentTick;
			HAL_GPIO_TogglePin(usr_ledRojo_GPIO_Port, usr_ledRojo_Pin);
		}
		*/

		/* --- Multitask con Planificador --- */

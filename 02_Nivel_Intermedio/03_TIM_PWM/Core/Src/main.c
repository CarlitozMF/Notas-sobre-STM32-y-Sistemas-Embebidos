...
void Task_RGB_Update(void) {
    // Llamada a la tarea del drivers
    RGB_LED_Task(&ledRGB, 30, 5);
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	/* 1. Configuración de la PAL (Puente Hardware-Software) */
	    hal_interface_t stm32_pal = {
	    ...
	    RGB_LED_Init(&ledRGB, ch_r, ch_g, ch_b, RGB_ANODE_COMMON, 700, stm32_pal);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, LD1_Pin|LD3_Pin|LD2_Pin|LED_HEART_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : LD1_Pin LD3_Pin LD2_Pin LED_HEART_Pin */
  GPIO_InitStruct.Pin = LD1_Pin|LD3_Pin|LD2_Pin|LED_HEART_Pin;
